...
data "coder_parameter" "repo_url" {
  ...
  default      = ""
  mutable      = true
  description  = "Git repository URL to clone (leave empty if prompt includes repo context)"
}
...
data "coder_parameter" "system_prompt" {
  ...
  form_type    = "textarea"
  mutable      = false
  description  = "System prompt for the background agent"
  default      = <<-EOT
    -- Framing --
    You are a background coding agent running inside a Coder Task workspace.
    Your job is to read a GitHub issue, implement the solution, and open a PR.
    ...
    -- Workflow --
    1. Use `gh issue view <url>` to read the full issue context
    2. Clone the repository if not already present
    3. Create a feature branch: feat/<issue-number>-<short-description>
    4. Implement the solution with minimal, correct, reviewable changes
    5. Run available checks (lint, typecheck, tests) and fix failures
    6. Commit with Conventional Commits (reference the issue number)
    7. Push and open a PR via `gh pr create` linking the issue
    8. Post a summary comment on the issue

    -- Guidelines --
    - Follow the project's CLAUDE.md / AGENTS.md if present
...
data "coder_workspace_preset" "github_task" {
  name    = "GitHub Task"
  default = true
  parameters = {
    repo_url      = ""
...
data "coder_workspace_preset" "heavy_task" {
  name = "Heavy Task"
  parameters = {
    repo_url      = ""
    system_prompt = data.coder_parameter.system_prompt.default
